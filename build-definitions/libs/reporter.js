        ...
        if(options.filter){
            result = result.filter((el) => {
                if(!el.error
                    ...
                    || el.error.code == 'W032' 
                    || el.error.code == 'W040'
                    || el.error.reason == "'define' is not defined." 
                    || el.error.reason == "'$' is not defined." 
                    || el.error.reason == "'require' is not defined."
                    || el.error.reason == "'kendo' is not defined."
                    || el.error.reason == "'document' is not defined."
                ){
                    return false;
                }else{
                    return true;
